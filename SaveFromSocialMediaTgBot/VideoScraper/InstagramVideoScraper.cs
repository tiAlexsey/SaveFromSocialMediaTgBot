...
public class InstagramVideoScraper(IConfiguration configuration, HttpClient client) : IVideoScraper
{
    ...
    private CookieParam[] Cookies { get; set; } = [];
    ...
    private async Task<string?> TryGetVideoUrlAsync(string pageUrl)
    {
        ...
        try
        {
            ...
            while (tryCount++ <= 1)
            {
                ...
                if (!match.Success)
                {
                    await page.SetCookieAsync(await AuthorizationAsync(login, password));
                }
                else
                {
                    tryCount++;
    ...
    private async Task<CookieParam[]> AuthorizationAsync(string login, string password)
    {
